...
model OFD_1Jan_HeatDem "OFD with TMC, TIR and TRY"
  import AixLib;
...
public
  Modelica.Blocks.Interfaces.RealOutput PHeatTot(unit="W") annotation (
      Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=270,
        origin={25,-110}),iconTransformation(extent={{172,-95},{188,-79}})));
protected
  AixLib.Building.Components.Weather.Weather Weather(Latitude = 49.5, Longitude = 8.5, GroundReflection = 0.2, tableName = "wetter", extrapolation = Modelica.Blocks.Types.Extrapolation.Periodic,                                                                                                 Wind_dir = false, Wind_speed = true, Air_temp = true,
    ...
    SOD=AixLib.DataBase.Weather.SurfaceOrientation.SurfaceOrientationData_N_E_S_W_RoofN_Roof_S())                                                                                                           annotation(Placement(transformation(extent={{-179,79},
            {-131,111}})));
equation
  // Romm Temperatures
  ...
  SolarRadiation[6] = Weather.SolarRadiation_OrientedSurfaces[6].I;
    ...
    PHeatTot = -OFD.GF_Hydraulic.Rad_Livingroom.Q_flow-OFD.GF_Hydraulic.Con_Livingroom.Q_flow
             ...
             -OFD.UF_Hydraulic.Rad_Children1.Q_flow-OFD.UF_Hydraulic.Con_Chidlren1.Q_flow
  ...
  connect(OFD.WindSpeedPort, Weather.WindSpeed) annotation (Line(points={{
          100.293,21.73},{100.147,21.73},{100.147,104.6},{-129.4,104.6}}, color=
         {0,0,127}));
  connect(Weather.AirTemp, OFD.Air_Temp) annotation (Line(points={{-129.4,99.8},
          {137.8,99.8},{137.8,21.73},{137.267,21.73}}, color={0,0,127}));
  connect(Weather.SolarRadiation_OrientedSurfaces, OFD.SolarRadiationPort)
    annotation (Line(points={{-167.48,77.4},{-167.48,55},{161,55},{161,17.05},{
          151.033,17.05}}, color={255,128,0}));
